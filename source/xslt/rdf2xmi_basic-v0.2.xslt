...
<xsl:stylesheet xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:owl="http://www.w3.org/2002/07/owl#" xmlns:dc="http://purl.org/dc/terms/" xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#" xmlns:sh="http://www.w3.org/ns/shacl#" xmlns:UML="http://schema.omg.org/spec/UML/2.5.1"  xmlns:xmi="http://schema.omg.org/spec/XMI/2.1" xmlns:typ="http://whatever" version="2.1" exclude-result-prefixes="typ">
	...
	<xsl:template match="/">
		...
		<xsl:comment>Generated with rdf2xmi-basic.xslt <xsl:value-of select="current-dateTime()"/></xsl:comment>
		...
		<!-- nota bene: xmi-versie 1.3 is is de laatste neutrale versie die in Enterprise Architect gelezen kan worden -->
			...
			<UML:Namespace.ownedElement>
				...
				<UML:Namespace.ownedElement>
					...
					<xsl:apply-templates select="//owl:DatatypeProperty" mode="richtype"/>
					<!-- enumerations -->
					<xsl:message>Enumerations:</xsl:message>
					<xsl:apply-templates select="//sh:NodeShape[exists(sh:in)]" mode="enum"/>
				</UML:Namespace.ownedElement>
				</UML:Package>
				...
				<UML:Namespace.ownedElement>
					...
					<xsl:apply-templates select="//sh:NodeShape[not(exists(sh:in))]" mode="richtype"/>
				</UML:Namespace.ownedElement>
				</UML:Package>
			...
			<xsl:comment> ## TAGGED VALUES FOR DATATYPES/CLASSES ##</xsl:comment>
			<!-- rich datatypes -->
			<xsl:apply-templates select="//owl:DatatypeProperty" mode="typedvalues"/>
			<!-- enumerations -->
			<xsl:apply-templates select="//owl:ObjectProperty" mode="typedvalues"/>
			<!-- classes -->
			<xsl:apply-templates select="//sh:NodeShape[not(exists(sh:in))]" mode="typedvalues"/>
	...
	<!-- rich datatypes -->
	<xsl:template match="owl:DatatypeProperty" mode="richtype">
...
<!--					<UML:ModelElement.taggedValue>
		...
		<xsl:for-each-group select="key('object', @rdf:about)/.." group-by="sh:path/@rdf:resource">
			<xsl:variable name="dt-core" select="typ:cast(sh:datatype/@rdf:resource)"/>
	
	
			<UML:Generalization subtype="{$dt-uri}" supertype="{$dt-core}" xmi.id="{generate-id(.)}">		
<!--				<UML:ModelElement.taggedValue>
...
-->			</UML:Generalization>
	...
	<xsl:template match="sh:NodeShape[exists(sh:in)]" mode="enum">

				<xsl:variable name="en-name" select="key('subject', sh:targetClass/@rdf:resource)/rdfs:label"/>
				<xsl:variable name="en-uri" select="key('subject', sh:targetClass/@rdf:resource)/@rdf:about"/>
		...
		<!--					<UML:ModelElement.taggedValue>
					...
					<UML:Classifier.feature>
						...
						<!-- fetch enumeration literals. -->
						<xsl:for-each select=".//rdf:first">
	
							<xsl:variable name="lt-name" select="key('subject', @rdf:resource)/rdfs:label"/>
							<xsl:variable name="lt-uri" select="key('subject', @rdf:resource)/@rdf:about"/>
		
							<UML:Attribute name="{$lt-name}" xmi.id="{$lt-uri}">
	<!--							<UML:ModelElement.taggedValue>
									<UML:TaggedValue tag="stereotype" value="Enumeratiewaarde"/>
									<UML:TaggedValue tag="rdfType" value="{$lt-uri}" xmi.id="{generate-id(.)}"/>									
								</UML:ModelElement.taggedValue>												
	-->						</UML:Attribute>
				...
				<!-- fetch with object-key en make distinct sh:PropertyShape/sh:datatype with for-each-group -->
				<xsl:for-each-group select="key('object', @rdf:about)/.." group-by="sh:path/@rdf:resource">
					<xsl:variable name="en-core" select="typ:cast(sh:datatype/@rdf:resource)"/>
					<UML:Generalization subtype="{$en-uri}" supertype="{$en-core}" xmi.id="{generate-id(.)}">		
		<!--				<UML:ModelElement.taggedValue>
							<UML:TaggedValue tag="ea_type" value="Generalization"/>
							<UML:TaggedValue tag="stereotype" value="Generalisatie"/>
							<UML:TaggedValue tag="Datum opname" xmi.id="EAID_FB3F1EDB_45C3_1445_8DCF_74AA807615B8">
	...
	<xsl:template match="sh:NodeShape[not(exists(sh:in))]" mode="richtype">
			<!-- easyrdf creates a subnode for a class being a superclass. strange. anyway makes next operation non distinct. Therefor extra operation-->
...
-->		<!--								<UML:TaggedValue tag="stereotype" value="Attribuutsoort"/>
		-->	
							...
							<xsl:otherwise>
								<xsl:if test="exists(key('subject', sh:node/@rdf:resource)/sh:in)">
								<xsl:variable name="at-type" select="key('subject', sh:class/@rdf:resource)/@rdf:about"/>
								<UML:Attribute name="{$at-name}" xmi.id="{$at-uri}" type="{$at-type}">
									<UML:ModelElement.taggedValue>
<!--										<UML:TaggedValue tag="type" value="{$at-type}"/>
-->		<!--								<UML:TaggedValue tag="stereotype" value="Attribuutsoort"/>
		-->
			...
			<xsl:for-each select="key('subject', sh:targetClass/@rdf:resource)/rdfs:subClassOf">
				...
				<!-- nota bene: dit zou een specifiek easyrdf oplossing kunnen zijn -->
				<xsl:variable name="gn-name" select="owl:Class/rdfs:label"/>
				<xsl:variable name="gn-uri" select="owl:Class/@rdf:about"/>
							
				<UML:Generalization subtype="{$cl-uri}" supertype="{$gn-uri}" xmi.id="{generate-id(.)}">		
	<!--				<UML:ModelElement.taggedValue>
						<UML:TaggedValue tag="ea_type" value="Generalization"/>
						<UML:TaggedValue tag="stereotype" value="Generalisatie"/>
						<UML:TaggedValue tag="Datum opname" xmi.id="EAID_FB3F1EDB_45C3_1445_8DCF_74AA807615B8">
	...
	-->			</UML:Generalization>
			...
			<!-- nota bene: maybe this construction is typical for easyrdf -->
			<xsl:if test="exists(sh:path/owl:ObjectProperty)">
			
				<xsl:variable name="as-name" select="rdfs:label"/>
...
<!-- RDF doesn"t have bidirectional relationships. You need two inversed propeties. That is not within the present scope. So the multiplicity for the target is undetermd -->
...
-->								</UML:AssociationEnd>
	...
	<!-- rich datatypes typed values -->
	<xsl:template match="owl:DatatypeProperty" mode="typedvalues">
		<xsl:variable name="dt-name" select= "rdfs:label"/>		
		<xsl:variable name="dt-uri" select= "@rdf:about"/>		
		<xsl:comment><xsl:value-of select="$dt-name"/></xsl:comment>
		<UML:TaggedValue tag="rdfType" xmi.id="{generate-id(.)}" value="{$dt-uri}" modelElement="{$dt-uri}"/>
		...
		<xsl:for-each-group select="key('object', @rdf:about)/.." group-by="sh:path/@rdf:resource">
			<xsl:if test="exists(sh:minLength)">
				...
				<UML:TaggedValue tag="minLength" xmi.id="{generate-id(sh:minLength)}"  value="{$minLength}" modelElement="{$dt-uri}"/>
			</xsl:if>
			<xsl:if test="exists(sh:maxLength)">
				<xsl:variable name="maxLength" select= "sh:maxLength"/>	
				<UML:TaggedValue tag="maxLength" xmi.id="{generate-id(sh:maxLength)}"  value="{$maxLength}" modelElement="{$dt-uri}"/>
			</xsl:if>
			<xsl:for-each select="sh:pattern">
				<xsl:variable name="pattern" select= "."/>	
				<UML:TaggedValue tag="pattern" xmi.id="{generate-id(.)}"  value="{$pattern}" modelElement="{$dt-uri}"/>
			</xsl:for-each>
		</xsl:for-each-group>
	</xsl:template>

	<!-- enumerations typed values -->
	<xsl:template match="owl:ObjectProperty" mode="typedvalues">
		...
		<xsl:if test="exists(rdfs:range)">
			...
			<xsl:comment><xsl:value-of select="$en-name"/></xsl:comment>
			<UML:TaggedValue tag="rdfType" xmi.id="{generate-id(.)}" value="{$en-uri}" modelElement="{$en-uri}"/>
			<!-- fetch with object-key and make distinct sh:PropertyShape constraints with for-each-group -->
			<xsl:for-each-group select="key('object', @rdf:about)/.." group-by="sh:path/@rdf:resource">
				<xsl:if test="exists(sh:minLength)">
					<xsl:variable name="minLength" select= "sh:minLength"/>	
					<UML:TaggedValue tag="minLength" xmi.id="{generate-id(sh:minLength)}"  value="{$minLength}" modelElement="{$en-uri}"/>
				</xsl:if>
				<xsl:if test="exists(sh:maxLength)">
					<xsl:variable name="maxLength" select= "sh:maxLength"/>	
					<UML:TaggedValue tag="maxLength" xmi.id="{generate-id(sh:maxLength)}"  value="{$maxLength}" modelElement="{$en-uri}"/>
				</xsl:if>
				<xsl:for-each select="sh:pattern">
					<xsl:variable name="pattern" select= "."/>	
					<UML:TaggedValue tag="pattern" xmi.id="{generate-id(.)}"  value="{$pattern}" modelElement="{$en-uri}"/>
				</xsl:for-each>
			</xsl:for-each-group>
		</xsl:if>
	</xsl:template>

	<!-- classes typed values -->

	<xsl:template match="sh:NodeShape[not(exists(sh:in))]" mode="typedvalues">
...
-->
		...
		<xsl:for-each select="key('subject', sh:targetClass/@rdf:resource)">

			<!-- easyrdf creates a subnode for a class being a superclass. strange. anyway that one should not be found here -->
			<xsl:if test="string(node-name(..))='rdf:RDF'">			
				<xsl:variable name="cl-name" select="rdfs:label"/>
				<xsl:variable name="cl-uri" select="@rdf:about"/>
				<xsl:comment><xsl:value-of select="$cl-name"/></xsl:comment>
				<UML:TaggedValue tag="rdfType" xmi.id="{generate-id(.)}" value="{$cl-uri}" modelElement="{$cl-uri}"/>
			</xsl:if>
		</xsl:for-each>
