...
<xsl:stylesheet xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:owl="http://www.w3.org/2002/07/owl#" xmlns:dc="http://purl.org/dc/terms/" xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#" xmlns:sh="http://www.w3.org/ns/shacl#" xmlns:UML="http://schema.omg.org/spec/UML/2.5.1"  xmlns:xmi="http://schema.omg.org/spec/XMI/2.1" xmlns:type="http://whatever" version="2.1" exclude-result-prefixes="type">
	...
	<xsl:template match="/">
		...
		<!-- nota bene: xmi-versie 1.3 is is de laatste neutrale versie die in Enterprise Architect gelezen kan worden -->
			...
			<!-- rich datatypes variant 1 & 2  -->
			<xsl:apply-templates select="//owl:DatatypeProperty" mode="datatypetags"/>
			<xsl:apply-templates select="//rdf:Description[rdf:type/@rdf:resource='http://www.w3.org/2002/07/owl#DatatypeProperty']" mode="datatypetags"/>
			<!-- enumerations variant 1 & 2-->
			<xsl:apply-templates select="//owl:ObjectProperty" mode="enumerationtags"/>
			<xsl:apply-templates select="//rdf:Description[rdf:type/@rdf:resource='http://www.w3.org/2002/07/owl#ObjectProperty']" mode="enumerationtags"/>

			<!-- classes/objecttypes variant 1 & 2 -->
			<xsl:apply-templates select="//sh:NodeShape[exists(sh:property)]" mode="classtags"/>
			<xsl:apply-templates select="//rdf:Description[rdf:type/@rdf:resource='http://www.w3.org/ns/shacl#NodeShape']" mode="classtags"/>
	...
	<xsl:template name="enumeration">
		<!-- focus = ObjectProperty -->
			...
			<!-- focus is Class -->
				...
				<xsl:variable name="en-uri" select="@rdf:about"/>
...
<!-- classes -->
	<xsl:template name ="class">
			<!-- focus is NodeShape -->
			<!-- filter: exists(sh:property) to exclude possible NodeShape for EnumerationClass-->
			...
			<xsl:for-each select="key('subject', sh:targetClass/@rdf:resource)/rdfs:subClassOf">
				...
				<!-- fetch generalization variant 1 -->
				...
				<!-- fetch generalization variant 2 -->
...
-->								</UML:AssociationEnd>
	...
	<!-- rich datatypes variant 1 -->
	<xsl:template match="owl:DatatypeProperty" mode="datatypetags">
		<xsl:call-template name="datatypetags"/>
	</xsl:template>

	<!-- rich datatypes variant 2-->
	<xsl:template match="rdf:Description" mode="datatypetags">
		<xsl:call-template name="datatypetags"/>	
	</xsl:template>

	<!-- rich datatypes -->
	<xsl:template name="datatypetags">
		<xsl:variable name="dt-name" select= "rdfs:label"/>		
		<xsl:variable name="dt-uri" select= "@rdf:about"/>		
	...
	<!-- enumerations and enumeration literals variant 1-->
	<xsl:template match="owl:ObjectProperty" mode="enumerationtags">
		<!-- just select enumeration, not associations-->
		<xsl:if test="exists(rdfs:range)">
			<xsl:call-template name="enumerationtags"/>	
		</xsl:if>
	</xsl:template>

	<!-- enumerations and enumeration literals variant 2-->
	<xsl:template match="rdf:Description" mode="enumerationtags">
		<!-- just select enumeration, not associations-->
		<xsl:if test="exists(rdfs:range)">
			<xsl:call-template name="enumerationtags"/>	
		</xsl:if>
	...
	<!-- enumerations and enumeration literals -->
	<xsl:template name="enumerationtags">
		<!-- focus = ObjectProperty -->
		<xsl:if test="exists(rdfs:range)">
			<xsl:variable name="en-name" select="key('subject', rdfs:range/@rdf:resource)/rdfs:label"/>
			<xsl:variable name="en-uri" select="key('subject', rdfs:range/@rdf:resource)/@rdf:about"/>		
